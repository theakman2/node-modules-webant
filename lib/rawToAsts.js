...
function rawToAsts(opts,callback) {
	...
	function getRequireData(raw,base,type) {
		...
		if (opts.aliases.hasOwnProperty(raw)) {
	...
	function _rawToAsts(filePath,sameAsChunk,complete) {		
		...
		if (!opts.handlers.hasOwnProperty(ext)) {
			ancestry.shift();
			complete("No handlers registered to handle extension: " + ext);
			return;
		}
		
		var canHandle = opts.handlers[ext];
