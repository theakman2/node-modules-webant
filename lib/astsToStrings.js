...
function astsToStrings(data,opts,callback) {
	...
	fs.readFile(__dirname + "/public/dist/" + fileName + ".js",function(err,content){
		...
		var srcAfter = '"' + jsStringEscape(urlExt) + '"';
		...
		var bootstrap = {
			filePath:opts.dest,
			content:content
				.replace(/\$\$\$ENTRY_CHUNK\$\$\$/g,'"'+jsStringEscape(entryLoc.chunk.toString())+'"')
				.replace(/\$\$\$ENTRY_MODULE\$\$\$/g,'"'+jsStringEscape(entryLoc.module)+'"')
				.replace(/\$\$\$SRC_BEFORE\$\$\$/g,srcBefore)
				.replace(/\$\$\$SRC_AFTER\$\$\$/g,srcAfter)
				.replace(/\$\$\$ENTRY_CHUNK_MODULES\$\$\$/g,_chunkToString(data[entryLoc.chunk]))
		};
