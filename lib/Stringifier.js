...
Stringifier.prototype = {
	...
	stringifyChunk:function(chunk,map){
		...
		while (i--) {
			...
			ret += this.stringifyNode(chunk.modules[i]);
			ret += "\n";
			ret += "}";
			if (i !== 0) {
				ret += ",";
		...
		for(var i = 0; i < len; ++i) {
			var data = {
				content:"window[" + chunkGlobalVar + "]"
					...
					+ " = "
					+ this.stringifyChunk(chunks[i],map)
					+ ";\n",
