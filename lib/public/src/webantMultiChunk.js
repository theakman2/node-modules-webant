(function(entryChunk){
	...
	function load(cId) {
	    ...
	    s.onload = s.onreadystatechange = function(){
			if (!s.readyState || /loaded|complete/.test(s.readyState)) {
				...
				loadStatus[cId] = 2;
				var call = [];
				for (var j = 0; j < onloadCallbacks.length; j++) {
					if (!getMissingChunkIds(onloadCallbacks[j][0]).length) {
						call.push([j,onloadCallbacks[j][0],onloadCallbacks[j][1]]);
					}
				}
				for (var k = 0; k < call.length; k++) {
					onloadCallbacks.splice(call[k][0],1);
					var data = call[k][1];
					if (typeof data[0] === "string") {
						data = [data];
					}
					call[k][2].apply(undefined,getExports(data));
				}
			}	    	
	    };
	...
	function require(data,cb) {
		...
		if (!missingChunkIds.length) {
			...
			cb.apply(undefined,getExports(data));
		} else {
