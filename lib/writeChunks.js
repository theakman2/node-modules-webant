...
function _stringifyChunk(chunk, isOnlyChunk, postProcess, map, chunkIdx) {
	var ret = "{";
	var i = chunk.length;
	while (i--) {
...
function stringifyChunk(chunk,chunkIdx,chunks,map,postProcess) {
	if (chunks.length > 1) {
		return _stringifyChunk(chunk,false,postProcess,map,chunkIdx);
	}
	return _stringifyChunk(chunk,true,postProcess,map);
}
...
function stringifyChunks(chunks,postProcess,map) {
	...
	for(var i = 0; i < len; ++i) {
		ret.push(stringifyChunk(chunks[i],i,chunks,map,postProcess));
	}
	return ret;
}
...
function _writeChunks(chunks,map,opts,callback) {
	...
	fs.readFile(__dirname + "/public/src/" + fileName + ".js",function(err,content){
		if (err) {
			callback(err);
			return;
		}
